...
function AddNumerosWithPolygon({
  ...
  const selectedNumerosCount = useMemo(() => {
    return t("selectedCount", { count: numerosSelected.length });
  ...
  return (
    <Pane>
      {data === null ? (
        <Alert intent="none" title={t("polygonHint")} marginBottom={32} />
      ) : (
        <>
          <Alert intent="none" title={t("polygonEditHint")} marginBottom={8} />
